...
Depending on your schema, you may not start from the query node in a subgraph. The integration tests in this situation involves mimicking the gateway/router. You can execute [a query against `_entities`](/federation/subgraph-spec#resolving-query_entities) to do integration tests. This looks like the following:
